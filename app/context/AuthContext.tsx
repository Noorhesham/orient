...
const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res.general_settings?.data && res.status !== false) {
        ...
        if (res.user2_settings?.data && res.status !== false) {
  ...
  // };
  return (
    <AuthContext.Provider value={{ generalSettings, userSettings, user2Settings, handleLogout, setLogin,  }}>
      {children}
